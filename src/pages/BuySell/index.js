...
const BuySell = (props) => {
      ...
      return (
        <>
          ...
          <p>
            <b>Total amount:</b>{" "}
            {isActive.buy
              ? data.commission / 100 == 0
                ? "₹" + (data.amount - data.debit)
                : "₹" +
                  (
                    data.amount -
                    [(data.amount - data.debit) * (data.commission / 100)]
  ...
  const filterValueByFirm = (e) => {
    if (isActive.buy) {
      let buyList = buyListAll.filter(
        (item) => item.firm_id?.toString() === e.value.toString()
      );
      filterValue(filterText, buyList);
    } else if (isActive.sell) {
      let sellList = sellListAll.filter(
        (item) => item.firm_id?.toString() === e.value.toString()
      );
      filterValue(filterText, sellList);
    } else if (isActive.all) {
      let allList = buySellListAll.filter(
        (item) => item.firm_id?.toString() === e.value.toString()
      );
      filterValue(filterText, allList);
    } else if (isActive.urd) {
      let urdList = buySellListAll.filter(
        (item) =>
          item.firm_id?.toString() === e.value.toString() && item["URD"] != 0
      );
      filterValue(filterText, urdList);
    }
  };
  ...
  const columns = useMemo(
    () => [
        ...
        selector: (row) => {
          ...
          return (
            <div className="user-wraps">
              ...
              <div className="c-date">{formatDate(row.date)}</div>
            </div>
          );
        },
        sortable: true,
      ...
      },
      {
        name: "Amount",
        selector: (row) =>
          row.amount ? "₹" + parseInt(row.amount).toLocaleString("en-IN") : "",
        sortable: true,
        hide: "md",
      },
      {
        ...
        selector: (row) => {
          return row.debit ? (
            <span className="badge rounded-pill bg-text text-bg-danger">
              {"₹" + parseInt(row.debit).toLocaleString("en-IN")}
            </span>
          ) : (
            ""
          );
      ...
      <div className="usermanagement-main">
        ...
        <div>
          <ul id="report-filter" className="report-filter tabs">
            <li
              className={`filter-item pending-r ${
                isActive.all ? "active" : ""
              }`}
            >
              <a
            ...
            <li
              className={`filter-item complete-r ${
                isActive.sell ? "active" : ""
              }`}
            >
              <a
                onClick={() => {
