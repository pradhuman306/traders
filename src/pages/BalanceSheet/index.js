                ...
                <p className="total-am">
                  ...
                  <label className={`badge rounded-pill bg-text ${totalGSTAmount < 0 ? " text-bg-success" : " text-bg-danger"}  xl-text`}>
                    ...
                    {makePositive(parseInt(totalGSTAmount)).toLocaleString(
                      "en-IN"
                    ) + `${totalGSTAmount < 0 ? " CR." : " DR."}`}
                  </label>
                </p>
                <p className="pending-am">
                  <span>Total Pending Amount</span>
                  <label className="badge rounded-pill bg-text text-bg-warning xl-text">
                    {parseInt(totalPendingAmount) < 0
                      ? priceFormatter(
                          makePositive(makePositive(totalPendingAmount))
                        ) + `${" CR."}`
                      : priceFormatter(
                          makePositive(makePositive(totalPendingAmount))
                        ) + `${totalPendingAmount < 0 ? " CR." : " DR."}`}
                  </label>
                </p>
              </div>
            </div>
