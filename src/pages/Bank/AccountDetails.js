...
import { dataFormatCSV, formatDate, makePositive, priceFormatter, titleCase } from "../../actions/common";
...
const AccountDetails = (props) => {
  ...
  const accountDetailsAll = useSelector(
    (state) => state.accountReducer
  ).accountDetails;
  const accountSingle = useSelector(
    (state) => state.accountReducer
  ).accountSingle;
  const dispatch = useDispatch();
  const [filterText, setFilter] = useState("");
  const [accountDetails, setList] = useState([...accountDetailsAll]);
  const [accountSingleDetails, setSingleDetails] = useState({
    ...accountSingle,
  });
          ...
          <p>
            ...
            {data.credit ? (
              <span className="credit">
                + {"₹" + parseInt(data.credit).toLocaleString("en-IN")}
              </span>
            ) : (
              ""
            )}
          ...
          <p>
            ...
            {data.debit ? (
              <span className="debit">
                - {"₹" + parseInt(data.debit).toLocaleString("en-IN")}
              </span>
            ) : (
              ""
            )}
    ...
    () => [
...

      {
        ...
        selector: (row) =>
          row.credit ? (
            <span className="credit">
              + {"₹" + parseInt(row.credit).toLocaleString("en-IN")}
            </span>
          ) : (
            ""
          ),
    ...
    <div className="body-content">
      ...
      <div className="mr-minus">
        <div className="usermanagement-main">
          ...
          {accountDetails.length?    <div>     <CSVLink data={dataFormatCSV(accountDetails, titleCase(accountSingleDetails.name), 'accountHistory')} filename={`${accountSingleDetails.name?.replace(" ", "_")}.csv`}
