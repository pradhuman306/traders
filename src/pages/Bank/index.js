...
const Bank = (props) => {
    ...
    const accountListAll = useSelector((state)=>state.accountReducer).accountList;
    const dispatch = useDispatch();
    const [filterText, setFilter] = useState("");
    const [accountList, setList] = useState([...accountListAll]);
    ...
    useEffect(() => {
        if (filterText) {
            let tmp = accountListAll.filter((item) => {
                if (
            ...
            setList([...accountListAll]);
        }
    }, [filterText,accountListAll]);
